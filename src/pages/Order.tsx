...
<label className="col-span-2 text-sm font-bold">Material<select aria-label="Sticker material" className="mt-2 w-full rounded-xl border border-border bg-card p-3" value={material} onChange={e => setMaterial(e.target.value)}>{materialData.map(m => <option key={m.value} value={m.value}>{m.label} — {m.description}</option>)}</select></label>
</div>            {/* Quantity */}
            <div>
          ...
          <MobileOrderAction regionId="configure" price={quantityValid ? `$${totalPrice.toFixed(2)}` : '—'} detail={`${effectiveQty || 0} stickers`} label={added ? 'Added!' : 'Add to Cart'} disabled={!quantityValid || (Boolean(artworkFile) && artworkStatus !== 'uploaded')} onClick={handleAddToCart} />
