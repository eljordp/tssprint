...
export default function PaymentStatus() {
  ...
  const refresh = useCallback(async () => {
    ...
    try { setInvoice(await quickBooksRequest('checkout-status', { id, token })); setError('') }
  ...
  return <section className="section-container max-w-3xl py-12 md:py-20 space-y-6">
    <h1 className="text-3xl md:text-4xl font-black">{paid ? 'Payment recorded' : review ? 'Your invoice needs a review' : 'Review your invoice'}</h1>
    <p className="text-muted-foreground">{paid ? 'QuickBooks confirms a payment against your invoice, and your order is saved. Artwork review and proof approval come next.' : 'Check the items, sales tax and final total before paying. Your order starts after payment is confirmed.'}</p>
    ...
    {invoice && <div className="rounded-2xl border border-border bg-card p-5 md:p-7 space-y-5">
      <p className="text-sm break-all">{invoice.orderId || `Invoice ${invoice.invoiceNumber || 'being prepared'}`}</p>
      <ul className="divide-y divide-border">{invoice.items.map((item, i) => <li key={i} className="py-3"><p className="font-bold">{item.category || item.name} · {item.option}</p><p className="text-sm text-muted-foreground">{item.size} · {item.quantity} batch(es){item.addOns.length > 0 && ` · ${item.addOns.map(a => a.name).join(', ')}`}</p></li>)}</ul>
      ...
      {review && <p role="alert">Please contact the shop before paying. We need to confirm the invoice details.</p>}
