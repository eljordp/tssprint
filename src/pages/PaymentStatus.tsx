...
export default function PaymentStatus() {
  ...
  return <section className="section-container max-w-3xl py-12 md:py-20 space-y-6">
    ...
    <p className="text-muted-foreground">{paid ? invoice.ownerTest ? 'Your $1 test payment is confirmed and saved. This test requires no artwork, printing or shipping.' : 'Your payment is confirmed and your order is saved. Artwork review and proof approval come next.' : 'Check the items, sales tax and final total before paying. Your order starts after payment is confirmed.'}</p>
    ...
    {invoice && <div className="rounded-2xl border border-border bg-card p-5 md:p-7 space-y-5">
      <p className="text-sm break-all">{invoice.orderId ? `Order ${invoice.orderId}` : invoice.paymentMode === 'wallet' ? `Checkout ${invoice.id}` : `Invoice ${invoice.invoiceNumber || 'being prepared'}`}</p>
      ...
      {paid && <p className="text-sm">Your confirmation is queued for {invoice.email}. If it doesn’t arrive, contact the shop with your order reference. {invoice.ownerTest ? 'This is a payment test only; do not fulfill it.' : 'Nothing prints until you approve the proof.'}</p>}
      <PaymentDisclosure />
    </div>}
