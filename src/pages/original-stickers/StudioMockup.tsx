...
import { createArtworkPreview, type ArtworkPreview } from '@/lib/artworkPreview'
...
export default function StudioMockup({
  ...
  const artworkUrl = preview?.url || null
      ...
      <div className="mt-5 flex flex-col sm:flex-row items-center gap-3 bg-card border border-border rounded-lg p-3">
        ...
        {artworkName ? (
          <div className="flex flex-col gap-1 text-sm sm:flex-row sm:items-center sm:gap-2">
            ...
            <span className="text-xs text-muted-foreground" aria-live="polite">{previewLoading ? 'Preparing preview…' : previewError || (preview?.pages ? `PDF preview · page 1 of ${preview.pages}` : 'Preview ready')}</span>
          </div>
        ) : (
          <p className="text-xs text-muted-foreground text-center sm:text-left">
            PDF, PNG, JPG, WebP and SVG previews
          </p>
        )}
      </div>
