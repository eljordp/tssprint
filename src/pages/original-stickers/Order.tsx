...
function Sticker({ shape, artworkUrl, size = 96, dashed = false }: { shape: string; artworkUrl: string; size?: number; dashed?: boolean }) {
  ...
  return (
    ...
    >
      {artworkUrl ? (
        <ResponsiveImage src={artworkUrl} alt="Artwork" className="w-full h-full object-cover" />
      ) : (
...
export default function Order() {
  ...
  const handleAddToCart = () => {
    ...
    if (editingItem) {
      ...
      navigate(searchParams.get('returnTo') === 'checkout' ? '/checkout' : '/cart')
    } else addItem(item)
      ...
      <section id="configure" className="py-8 md:py-12 scroll-mt-24">
        <div className="section-container">
          ...
          >
            {/* Shape */}
            <div>
              <h2 className="text-sm font-black uppercase tracking-wider mb-3">Shape</h2>
            ...
            {/* Material */}
            <div>
              <h2 className="text-sm font-black uppercase tracking-wider mb-3">Material</h2>
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="flex items-center gap-2">
                        ...
                        {disc > 0 && (
                          <span className={`text-xs font-semibold ${isActive ? 'text-primary-foreground' : 'text-green-400'}`}>
                            -{disc}%
                          </span>
                        )}
                      </span>
                ...
                <div className={`rounded-xl border transition-all ${customQty ? (effectiveQty < MIN_QTY ? 'border-yellow-500/40 bg-yellow-500/5' : 'border-primary bg-primary/10') : 'border-border'}`}>
                  ...
                  <div className="flex items-center gap-2 px-3 pb-3">
                    <input
                      ...
                      className="min-w-0 flex-1 bg-muted rounded-lg px-3 py-2 text-base text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/20"
                    />
          ...
          >
            ...
            <fieldset className="bg-card border border-border rounded-2xl p-8 flex flex-col items-center justify-center text-center">
              ...
              <h2 className="font-bold text-lg mb-1">How will you provide artwork?</h2>
              ...
              <div className="space-y-3 w-full max-w-xs">
                ...
                >
                  ...
                  <span>{artworkFile || artworkUpload ? 'Replace uploaded artwork' : 'Upload my artwork now'}</span>
                </button>
                <button
                  type="button"
                ...
                />
                {(artworkFile || artworkUpload) && (
                  <p className="text-xs text-primary font-medium mt-2">
                    &#10003; {artworkFile?.name || artworkUpload?.fileName}
                  </p>
                )}
                ...
                {artworkFile && !artworkUrl && (
                  <p className="text-xs text-muted-foreground mt-1">
                    {previewLoading ? 'Preparing artwork preview…' : previewError || 'A preview is not available for this file.'}
                  </p>
                )}
                {artworkIntent === 'send_later' && (
            ...
            <div className="bg-card border border-border rounded-2xl p-6 flex flex-col items-center">
              ...
              <p className="text-xs text-muted-foreground mt-2">
                {previewLoading ? 'Preparing artwork preview…' : preview?.pages ? `PDF preview · page 1 of ${preview.pages}` : artworkUrl ? 'Preview of your design' : 'Upload artwork to see your design applied'}
              </p>
            </div>
            ...
            <div className="bg-card border border-border rounded-2xl p-6 flex flex-col">
              ...
              >
                {effectiveQty < MIN_QTY ? (
                  ...
                  <><ShoppingCart size={18} /> {editingItem ? 'Save changes' : 'Add to Cart'}</>
                )}
              </button>
              {effectiveQty > 2500 && (
                <a
