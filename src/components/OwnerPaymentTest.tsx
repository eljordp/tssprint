...
export default function OwnerPaymentTest() {
  ...
  async function prepare() {
    ...
    try {
      ...
      const saved = sessionStorage.getItem('tss_owner_payment_test')
      ...
      sessionStorage.setItem('tss_owner_payment_test', JSON.stringify(attempt))
      const response = await fetch('/api/quickbooks/owner-payment-test', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session.access_token}` }, body: JSON.stringify({ id: attempt.id, token: attempt.token, ga4 }) })
      ...
      if (!config.applePay?.enabled || !config.applePay.clientId) throw new Error('Apple Pay is unavailable.')
  ...
  return <details className="border-t border-border pt-4" open={new URLSearchParams(window.location.search).get('owner_test') === '1'}>
    ...
    <div className="space-y-4 pt-4">
      ...
      {!debug && <a className="text-primary underline" href="/admin?tab=quickbooks&owner_test=1&analytics_debug=1">Enable tracking verification</a>}
