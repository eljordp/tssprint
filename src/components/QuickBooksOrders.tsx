...
export default function QuickBooksOrders({ request }: { request: (action: string, method?: string) => Promise<{ checkouts?: Row[]; checked?: number; worker?: WorkerHealth }> }) {
  ...
  return <section className="space-y-3 border-t border-border pt-4">
    ...
    <p className="text-sm text-muted-foreground">Card and Apple Pay checkouts stay here with their payment and recovery status. PayPal accounting imports run through the existing connector. Email “accepted” means the email provider accepted it, not that it reached the inbox.</p>
