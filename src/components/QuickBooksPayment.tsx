...
export default function QuickBooksPayment({ disabled, payload, onBusy, onError }: {
  disabled: boolean; payload: () => Record<string, unknown>; onBusy: (busy: boolean) => void; onError: (message: string) => void
}) {
  ...
  useEffect(() => {
    let cancelled = false
    fetch('/api/quickbooks/checkout-config').then(r => r.json()).then(data => { if (!cancelled) setAvailable(data.enabled === true) }).catch(() => {})
