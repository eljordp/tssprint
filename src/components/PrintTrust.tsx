...
export default function PrintTrust() {
  ...
  return <aside className="rounded-2xl border border-border bg-card p-4 my-5">
    ...
    <div className="grid grid-cols-3 gap-2 mt-3">{examples.map(project => <button key={project.slug} type="button" onClick={() => setSelected(project)} aria-label={`View ${project.title}`} className="text-left rounded-lg overflow-hidden border border-border"><img src={project.image} alt={project.title} className="aspect-square w-full object-cover" loading="lazy" /><span className="block p-2 text-[11px] font-semibold">{project.title}</span></button>)}</div>
