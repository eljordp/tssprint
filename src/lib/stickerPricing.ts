...
type BasePricing = { basePrices: { maxQty: number; price: number }[] }
const roundMoney = (amount: number) => +amount.toFixed(2)

export function isValidStickerQuantity(quantity: number): boolean {
 ...
 */
export function getBasePrice(quantity: number, config: BasePricing): number {
...
export function getStickerPrice(quantity: number, config: BasePricing, sizeMultiplier = 1, materialMultiplier = 1) {
