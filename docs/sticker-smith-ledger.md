...
### Recovery and customer email release — September 14, 23:59 PDT

Live: **dpl_Aopf8CJLhrYFyCNWgyUZ3aPNsEny**, `https://tssprint-7kmi88kwp-jordis-projects-94d2df39.vercel.app`, application **7b4139c**, recovery change **13e53f0**, combined merge **491d4ec**, prerender record **d2c0240**. tssprint.com alias confirmed READY. Preserves committed 961991a gated QuickBooks work. No payment or checkout database changes performed by this task.

- Recovery now shows products, configuration, artwork filename/intent and pre-discount subtotal before replacement. Keeping the current cart preserves it. Lookup remounts when the link changes, preventing stale results. Unavailable/expired links offer useful next steps; transient errors retain retry. Empty-cart exit returns to stickers. Email-my-cart exposes actionable errors and guards duplicate clicks.
- Two controlled cart messages reached **eljordp@gmail.com**, with Inbox placement observed. First live test restored Matte 50pcs + Clear 100pcs across Chrome→IAB, retaining the synthetic `vector.pdf` production attachment and send-later choice: subtotal $134.30, AUTO10 $13.43, total $120.87; items persisted through reload. Cross-browser thumbnail remains outside the preview guarantee. One pending synthetic PDF object created; no order, quote or payment.
- Initial recovery sync subsequently returned Invalid cart link. A narrow read-only SQL check found the original emailed cart was now empty/email NULL; cause of clearing was not established. The restored local items persisted. Do not call this a proven app bug or tracking pass. Second stage test verified a fresh emailed link, item review, keep-current action, restoration, and $47.50/$42.75 pricing. Both staged source/destination test items removed normally afterward.
- Password: initial request had no matching customer account, so generic success was not evidence of sending. User created eljordp customer account; confirmation arrived and actual link opened My Account / Jordi P. A second reset request after confirmation delivered the branded email and reached Choose a new password. **User password save/login and reused-link check remain pending.** Agent did not enter/read passwords.
- User requested a proper welcome email. Canonical `supabase/templates/confirm-signup.html` and `reset-password.html` are saved in the corresponding Supabase templates, with subjects in README. Actual logo at `https://tssprint.com/brand/sticker-smith-logo.png` returns HTTP200 image/png; normal browser rendering verifies its 400px source image loads. Welcome explains account history/profile/referrals/artwork and includes a clear confirmation CTA; reset uses matching styling and one-use guidance. First branded reset inbox delivery preceded the final logo refinement; final logo versions have persisted-template/browser-render evidence, not a new inbox-delivery claim.
- Supabase preview's broken logo is explained by its response Content-Security-Policy: img-src permits Supabase and selected domains, excludes tssprint.com; ordinary email HTML rendering loads the same URL correctly. Query-string cache-bust did not help and was discarded. Do not weaken security protections to change the dashboard preview. Check final actual-client rendering on the next permitted email.
- Editor lesson: Monaco fill appended to existing text. Select all + Backspace through editor keys, then fill; verify one message in preview. Supabase's floating banner covered Save; dismiss it and verify Save disables and subject persists after reload. Duplicate default content was removed before final publication.
...
### Mobile cart and recovery report — September 15, 01:10 PDT

Scope: user authorized both admin recovery reconciliation and the mobile cart/edit/checkout pass. Change **4b3bd05** preserves `is_test` on later saves and inherits it from a validated emailed-cart source. Previously a later save without the QA marker could move a controlled test into the customer list. Admin now defaults to Customer carts with a separate Test carts view, and each record exposes cart/source IDs, email acceptance, restoration time and recorded payment. Tests are excluded from customer totals. Find it under **Admin → Reports → Sales & traffic → Cart recovery**; Tracking checks help now points there (aa72bfa). The older Orders cart view remains available.

Mobile: editing a sticker from Cart had ignored `returnTo=cart` and jumped to checkout. It now returns to the same cart item; checkout-origin edits still return to checkout. Edit links have 44px minimum targets. A fixed mobile total/Checkout bar keeps the next step accessible without scrolling through every item. A staged screenshot caught the older checkout button still visible because its button styling overrode `hidden`; c7e79e3 moves responsive hiding to a wrapper.

Observed live before release: two-item Matte/Clear editing changed only the selected line, retained the synthetic vector.pdf production attachment, and kept name/email/pickup through checkout → edit → save. Pickup removes shipping-address fields. Browser text snapshots hid the email value; screenshots showed the entered address before and after editing, so **no email-loss defect was established**. Cart/checkout showed no horizontal overflow at 390px. Staged cart edit 50 Matte 2x2 → 3x3 returned to the cart with $61.75 subtotal/$55.57 discounted total; increasing to two batches gave 100 pieces/$123.50 subtotal/$111.15 total; removing the staged test item reached the empty-cart state. No payment, invoice, customer email or new artwork upload in this pass.

Combined automated verification: **29 tests pass** covering recovery/token/API/request/editing, payment-policy/delivery preservation and QuickBooks worker behavior. Scoped ESLint, TypeScript/build and 33-route prerender passed. Latest combined application commit **22eef01** preserves the other task's committed a948045 checkout/payment work. Final release evidence follows below.

Tracking checks at 01:03 PDT showed recent product-view, upload-start/success, add-to-cart, begin-checkout and phone-click signals. Upload-failure, artwork-choice, shipping, payment and quote-submit milestones had no recent matching signal. This does not establish failure; QA sessions are excluded, and this report covers the latest 500 matching events. GA4 configured ID G-4B9FXT1HQ9 remains configuration evidence only; no fresh GA4 delivery claim from this pass. Automated abandoned-cart campaigns remain outside this verified customer-requested recovery flow. Ratings remain customer7.75, tracking6 provisional, admin7.5 provisional pending fuller outcome/performance evidence. The 602-entry Drive review remains deferred.


Release verification: **dpl_BXbVEVKWJ6L5YL3vkieFZN8NcYZE**, https://tssprint-m72eonyj8-jordis-projects-94d2df39.vercel.app, application **22eef01**, promoted successfully to tssprint.com around 01:08 PDT. Staged final build verified one visible mobile Checkout link and 382px content/viewport at a 390px browser setting. Live DOM verified a single mobile checkout link; live desktop cart edit returned to the same item anchor and updated Matte 3x3 → 2x2 while retaining vector.pdf and leaving Clear250 unchanged ($212 subtotal/$190.80 discounted total). Final staged/live temporary cart items were removed via cart/checkout controls. User account tab was preserved; viewport overrides reset. IAB screenshots briefly duplicated/stitched after viewport changes; do not treat that automation artifact as a verified site defect or claim a final live 390px screenshot pass.

Live recovery reconciliation (01:08–01:10 PDT): Test carts view loaded 10 test records separately from 11 default customer/legacy records. Our active two-item cart **fa1a8892-44ac-4952-86c7-7947248569b5**, previously misclassified, appeared under Test carts with no payment. Customer view returned 0 verified inactive / $0 inactive subtotal / 0 paid cart-linked orders; old empty and Legacy (unverified) rows remain visible, so this is not a claim that no historical customer abandoned a cart. No blanket relabeling of legacy rows performed.

The prior second controlled email source **4206ce56-2a0d-4e28-a0b9-86aaabba6cb4** shows email accepted **September14 23:50:06 PDT**, restored **23:51:27 PDT**, and no recorded payment. Destination **e3190d0e-e201-4e3a-bfe9-5a4d268d5778** references that source and has no payment. Both remain test records after their earlier item cleanup. This closes the previously pending admin/source reconciliation for the successful second recovery journey. The first source **7b337f2f-27d1-45d4-a0e5-c9194aec0c98** still has no restoration timestamp; its current email/items changed since the earlier test, so do not reinterpret that first failed sync as a proven success. No new message was sent by this task.

Remaining: independently verify GA4 receipt/attribution and missing customer milestones; complete an authorized paid purchase/receipt/admin/purchase-event reconciliation when the payment setup is ready; broader keyboard/device/performance checks and exact product/material evidence. A full automation campaign is not enabled or verified by this pass. No score increase solely for shipping these fixes.

### GA4 receipt, accessibility and performance — September 15, 01:33 PDT

Published customer navigation/contrast/local-font changes **6b57675**, combined **ddca322**, as **dpl_FrL9gVRTV6xv11FyEV4SLYueWLHn** at tssprint.com. Preserves the other task's public QuickBooks launch through4615c98/9e28d59. [Full dated evidence and report links](./accessibility-performance-2026-09-15.md).

Independent GA4 DebugView receipt confirmed product view, artwork choice, add-to-cart, checkout and delivery choice, plus codex/verification campaign attribution and the actual sticker item. No purchase claim. The initial begin_checkout value captured $47.50 before AUTO10 resolved to $42.75; fix promo-resolution timing before treating that value as final. Both GA4 traffic filters remain Testing; regular reports may still contain QA. Removed this task's one send-later test cart item; no new email/upload/invoice/payment.

Shipped keyboard Services disclosure, mobile modal focus/Escape/close behavior, skip navigation, visible focus rings, stronger blue-button contrast (2.04:1→9.10:1), corrected sticker/footer headings, and self-hosted/preloaded Noto Sans. Local keyboard and390px visual checks passed; staged/live source controls verified. Scoped lint, TypeScript/build,33-route prerender and33 combined checkout/analytics/QuickBooks tests passed.

PageSpeed sticker score before→after: **mobile performance68→68, desktop92→95, automated accessibility95→100 on both**. Mobile FCP3.2→2.3s andLCP6.6→6.2s, but TBT20→240ms; no blanket performance-success claim. Signage mobile baseline **69 performance /100 accessibility**,LCP10.1s, with product-image loading/rendering and937KiB estimated image savings to investigate. Lab measurements are not overall customer ratings or real-device certification. Full reports, caveats and exact times are in the linked audit.

Current customer7.75, tracking6 provisional, admin7.5 provisional remain conservative overall assessments. Next: responsive images/route bundles and broader real-device/screen-reader checks; promo-settled checkout tracking and GA4 test-filter decision; server measurement acknowledgement/key and authorized paid-order/receipt/purchase reconciliation; exact material facts/matched real photography. Existing invoice3275 remains unpaid at$133.86 including$12.99tax; no charge made.602-entry Drive review deferred.


### Mobile image delivery, checkout discounts and deployment — September 15, 09:40 PDT

Released 00cbfe0 + 6fa67a9 as **dpl_Ah9LeaFxJynL9tcfLLotjntTDYNr** to tssprint.com. Existing photographs now have responsive WebP sizes, ordering content appears without its entrance fade, initial JS is smaller and route chunks preload. Checkout promo readiness gates quoting, payment and begin_checkout so AUTO10 and its amount resolve together. 43 targeted tests plus two cross-platform image-reference tests passed; build, 33-route prerender and scoped lint passed. A failed first stage was never promoted; corrected dimension-based asset matching passed the remote build.

Measured mobile performance: stickers **68 → 71**, signage **69 → 72**; signage LCP **10.1 → 6.2s**. Desktop stickers **95 → 96**. Automated accessibility/Best Practices/SEO remain 100. Mobile speed is still unfinished: delayed H1 rendering and unused initial JS remain, and lab runs do not establish real-user Core Web Vitals. Live phone-sized IAB check confirmed actual responsive pictures and no sideways overflow. Staged checkout confirmed AUTO10 $42.75 on $47.50 subtotal; removed the test item, no invoice/email/payment.

QuickBooks remains publicly enabled with the prior eight category mappings. Invoice3275 still unpaid ($133.86 including $12.99 tax); no receipt/purchase pass claimed. User's screenshot concerned failed df89084 tax-mapping deployments, not a live outage; that newer mapping is not included here. GA4 acknowledgement gate disappeared; API-secret creation/configuration is prepared and awaits specific confirmation. No other task contacted during this continuation.

Full sources, before/after metrics, release details and unresolved rendering work: [accessibility-performance-2026-09-15.md](accessibility-performance-2026-09-15.md). Customer/admin/tracking ratings remain **7.75 / 7.5 provisional / 6 provisional**; PageSpeed is a separate scale. The 602-entry Drive review remains deferred.

### September 15, 2026 — 10:30 PDT: full mobile pass, GA4 configuration and payment handoff

See [mobile audit and verification record](mobile-audit-2026-09-15.md). Checked 36 route layouts plus narrow-phone upload/cart/checkout and navigation/form flows. First performance release is live (`9204428`, `dpl_5zkJKUPK6ry4CMcUeEtGaWyWfsAB`): signage 72→87, stickers 70→72, home 73→74. Follow-up hydration and typo-tolerant search changes are being verified before final promotion; do not substitute staged behavior for live scores. GA4 server credential is configured and live admin confirms readiness. Correct the earlier “new tax mappings excluded” status: they are now included. Invoice 3277 ($47.35, eljordp test inbox) is prepared but unpaid; paid receipt and GA4 purchase remain open until actual payment. The 602 Drive entries remain deferred. No other tasks contacted.


### September 15, 2026 — 11:00 Pacific · Original sticker layout, preview repairs and mobile speed

User confirmed Cal prefers the exact old sticker arrangement and authorized repairing previews/cart behavior inside it. Merged the live historical restoration before publishing; kept four option columns, circular material swatches, and separate artwork/preview/summary panels. Both preview tools now render PDF first pages; verified JPG/PNG/SVG/WebP and PDF-compatible AI, with explicit EPS limitations. Staged PDF upload survives add/edit/reload; changing 50→100 retains the same cart item and attachment, and replacement PNG persists. Exact price cents, saved preset highlighting, readable cyan-button labels, 16px quantity input and existing artwork-funnel events are retained. Typo/plural search is live, including “stikcers,” “buisness cards,” and a Sticker Sheets result.

Published functional release `78371f2`, then startup release **`937f693` / `dpl_9jxQa2YczqAh2v3PSD61aqvuMP9Z`**. Home mobile Lighthouse **73→94**, signage **72→90**; respective LCP 2.4s/2.9s. Final sticker retry succeeded at 11:01 Pacific: **70→94** (71 immediately after historical restoration), LCP **6.2s→2.6s**, CLS 0. All three final reports show accessibility/best practices/SEO 100; these are cold lab measurements, not every-device guarantees. Three entry pages now include their styles in the HTML and can paint before hydration. Thirty targeted regression tests plus later 13 overlapping checks, TypeScript, scoped lint/build, 320px live verification pass. All 36 customer routes received 390px layout coverage earlier; physical device behavior is not claimed verified.

GA4 Production server credential is configured and confirmed in Admin → Settings → QuickBooks → Check payment settings. New taxable QuickBooks website product mappings are included. Invoice **3277**, **$47.35**, to the authorized test inbox still awaits user payment; paid order, receipt delivery and GA4 purchase cannot yet be marked verified. Other invoices were untouched. Full chronological findings, report links, limitations and testing evidence: [mobile-audit-2026-09-15.md](mobile-audit-2026-09-15.md). The deferred Drive review remains deferred.
