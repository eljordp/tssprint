...
Status: on-site card checkout is deployed and enabled on tssprint.com. Real-money charge, receipt delivery, GA4 purchase and bank settlement remain unverified. On-site Apple Pay is implemented and locally tested, but not deployed/enabled; the Supabase SQL session must be restored to install its migration. Updated 2026-09-15.
...
1. Restore Supabase dashboard access. It redirected to an empty Organizations page, then GitHub sign-in. Database REST reads still work. The user has been asked to sign in; do not request a real purchase yet.
2. Apply `supabase/migrations/20260915220000_paypal_wallet.sql` transactionally. Verify wallet columns, unique order reference, service-only finalizer, existing table RLS, and scheduler dispatch covering wallet attempts. Test rejection of a finalization without completed capture evidence.
3. Staged source **0f86895** is READY in deployment **dpl_9y2RDMGM48gSjSAUn9VAbTUHKwYC** (`tssprint-d7kap7spq-jordis-projects-94d2df39.vercel.app`), with explicit deployment override `PAYPAL_APPLE_PAY_ENABLED=false`. It is **not promoted**. Config verifies cards enabled, wallet disabled; wallet capture rejects while disabled; PayPal admin verification rejects unsigned requests. After migration, enable `PAYPAL_APPLE_PAY_ENABLED=true` in the prepared production deployment; verify production quote → PayPal order creation/GET contract without authorizing/capturing money. Verify existing native card behavior and malformed/tampered requests.
4. Promote preserving the latest production source. Verify the live Apple SDK eligibility, official button and merchant validation on `tssprint.com`; stop before user payment approval. Register any additional exact checkout hostname only if needed.
5. User chooses an actual order and approves a real payment. The current old verification cart is **$181.91**, not a nominal $1 test. Verify one processor capture, one saved paid order with artwork, receipt arrival, matching GA4 transaction, and one connector accounting import with tax/fees. Do not independently create an accounting sale or refund without authorization.
