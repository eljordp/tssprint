...
3. Stage/build this source with the wallet flag off. After migration, enable `PAYPAL_APPLE_PAY_ENABLED=true` in the prepared production deployment; verify production quote → PayPal order creation/GET contract without authorizing/capturing money. Verify existing native card behavior and malformed/tampered requests.
