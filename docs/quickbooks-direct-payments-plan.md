...
- Verified in the signed-in The Sticker Smith QuickBooks company: **PayPal Connector by QuickBooks is connected**. Its settings show automatic posting ON, product tracking ON, customer tracking ON and supplier tracking ON. Existing imported payment rows include website sticker item descriptions. No connector settings, transactions or account mappings were changed.
- This changes the planned wallet accounting path: do not add an independent sales/invoice writer alongside this automatic connector. Intuit's [connector documentation](https://quickbooks.intuit.com/learn-support/en-us/help-article/mobile-apps/use-paypal-connector-quickbooks-app/L5eoHQvLj_US_en_US) describes incoming sales as sales receipts and supports matching existing invoices, but does not establish guaranteed automatic matching for this site. Use the connector as the accounting importer; investigate a non-posting tax quote (for example a tested QuickBooks Estimate) for wallet checkout so preparing tax does not create duplicate invoice revenue. The current wallet core is only a prepared contract and will need adaptation to the verified quote path. Do not enable it yet.
...
3. Resolve direct Apple Pay processing with Intuit. The public Tokens/Charges references and latest release notes inspected do not document an Apple Pay payload. This is an unresolved capability, not proof of universal non-support. No Apple Pay button has been added to the direct card form.
